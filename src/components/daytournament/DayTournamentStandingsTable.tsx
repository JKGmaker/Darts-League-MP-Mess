...
import { DayTournamentStandingsRow } from '@/types';

interface DayTournamentStandingsTableProps {
  standings: DayTournamentStandingsRow[];
}

export default function DayTournamentStandingsTable({ standings }: DayTournamentStandingsTableProps) {
  return (
        ...
        <table className="w-full min-w-[700px] text-left border-collapse text-sm">
          <thead>
            <tr className="bg-charcoal-950 text-emerald-400 font-semibold border-b border-emerald-900/50 sticky top-0">
              ...
              <th className="py-3 px-4 text-center text-emerald-300 font-bold bg-emerald-950/20">Pts</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-emerald-950/40 text-gray-200">
            {standings.length === 0 ? (
              <tr><td colSpan={9} className="py-8 text-center text-gray-500">No entries yet.</td></tr>
            ) : (
              standings.map((row, index) => {
                const isTopThree = index < 3;
                return (
                  <tr key={row.competitorId} className={`hover:bg-emerald-950/20 transition-colors ${index % 2 === 0 ? 'bg-charcoal-900/40' : 'bg-charcoal-950/20'}`}>
                    <td className="py-3.5 px-4 text-center font-bold">
                      <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs ${
                        index === 0 ? 'bg-emerald-400 text-charcoal-950 shadow-md shadow-emerald-500/20'
                        : index === 1 ? 'bg-slate-300 text-charcoal-950'
                        : index === 2 ? 'bg-emerald-700 text-white'
                        : 'text-gray-400'
                      }`}>{row.position}</span>
                    </td>
                    <td className={`py-3.5 px-4 font-medium ${isTopThree ? 'text-white' : 'text-gray-300'}`}>{row.name}</td>
                    ...
                    <td className={`py-3.5 px-3 text-center font-mono font-semibold ${row.legDifference > 0 ? 'text-emerald-400' : row.legDifference < 0 ? 'text-rose-400' : 'text-gray-400'}`}>
                      {row.legDifference > 0 ? `+${row.legDifference}` : row.legDifference}
                    </td>
                    <td className="py-3.5 px-4 text-center font-mono font-bold text-base text-emerald-300 bg-emerald-950/10">{row.points}</td>
                  </tr>
                );
              })
            )}
