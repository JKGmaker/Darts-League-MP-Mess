...
export const metadata = {
  title: 'MP Mess One-Day Tournaments',
  description: 'One-day darts and pool tournaments for MP Mess — singles, doubles, knockouts and leagues.',
};
...
function statusBadge(status: DayTournament['status']) {
  switch (status) {
    case 'active':
      return <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-400 bg-emerald-950/50 px-2 py-0.5 rounded-full">Active</span>;
    case 'completed':
...
export default async function DayTournamentHomePage() {
  ...
  return (
    <div className="min-h-screen bg-charcoal-950">
      <header className="relative min-h-[45vh] flex items-end overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-charcoal-950 via-charcoal-950/60 to-emerald-950/40" />
        <div className="relative z-10 w-full max-w-6xl mx-auto px-6 pb-12 pt-24">
          ...
          <h2 className="text-4xl sm:text-6xl font-black text-emerald-400 tracking-tight leading-none drop-shadow-lg">One-Day Tournaments</h2>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-6">
        ...
        {tournaments.length === 0 ? (
          <p className="text-gray-500 text-center py-10">No one-day tournaments yet — check back soon.</p>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2">
            {tournaments.map((t) => (
              <Link key={t.id} href={`/tournament/${t.id}`}
                className="bg-charcoal-900 border border-emerald-950/60 rounded-xl p-4 flex items-center justify-between gap-3 shadow-md hover:bg-charcoal-800 transition-colors">
                <div>
                  <p className="text-base font-bold text-white">{t.name}</p>
                  <p className="text-xs text-gray-500 mt-1 capitalize">
                    {t.sport === 'darts' ? '🎯' : '🎱'} {t.sport} · {t.entry_type} · {t.format}
                    {t.event_date && ` · ${new Date(t.event_date + 'T00:00:00').toLocaleDateString('en-IE', { day: 'numeric', month: 'short', year: 'numeric' })}`}
                  </p>
                </div>
